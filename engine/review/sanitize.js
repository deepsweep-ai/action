...
export const MAX_RENDERED_FIELD_LENGTH = 200;
/** Max characters per JSONL event field (ADR-004 contract — see above). */
export const MAX_EVENT_FIELD_LENGTH = 512;
/** The one strip set. Never redefine elsewhere — the guard test enforces it. */
const STRIP = /[\u0000-\u001F\u007F-\u009F\u202A-\u202E\u2066-\u2069]/g;
function sanitizeWithCap(value, cap) {
    let out = value.replace(STRIP, "");
    if (out.length > cap) {
        out = `${out.slice(0, cap)}…`;
    }
    return out;
...
export function sanitizeField(value) {
    return sanitizeWithCap(value, MAX_RENDERED_FIELD_LENGTH);
}
/**
 * Tail characters preserved by sanitizeFieldKeepEnding — comfortably larger
 * than the longest remediation ending. Sized by the S1.12 audit: the longest
 * is identity.regenerated's "re-review before continuing to trust attribution
 * in this environment." (70 chars); pin.drift's "re-pin via --update-baseline."
 * (31) and every audited warning ending fit well inside. This is a parameter
 * of the ONE human-render implementation (see module doc) — the event surface
 * is untouched by it.
 */
const KEEP_ENDING_TAIL = 80;
/**
 ...
 * are unchanged.
 */
export function sanitizeFieldKeepEnding(value) {
    const out = value.replace(STRIP, "");
    if (out.length <= MAX_RENDERED_FIELD_LENGTH)
        return out;
    const head = MAX_RENDERED_FIELD_LENGTH - KEEP_ENDING_TAIL;
    return `${out.slice(0, head)}…${out.slice(out.length - KEEP_ENDING_TAIL)}`;
}
/**
 ...
 * implementation.
 */
export function sanitizeEventField(value) {
    return sanitizeWithCap(value, MAX_EVENT_FIELD_LENGTH);
}
/**
...
export function sanitizeJsonValue(value) {
    if (typeof value === "string") {
        return sanitizeEventField(value);
    }
    ...
    if (typeof value === "object" && value !== null) {
        ...
        for (const [k, v] of Object.entries(value)) {
            out[sanitizeEventField(k)] = sanitizeJsonValue(v);
        }
        return out;
    }
    return value;
